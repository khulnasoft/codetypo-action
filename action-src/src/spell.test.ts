...
import { CodeTypoReporterForGithubAction, Logger } from './reporter.js';
...
describe('Validate Spell Checking', () => {
    test('Linting some files', async () => {
        ...
        const reporter = new CodeTypoReporterForGithubAction('none', { ...rOptions, verbose: false }, logger);
    ...
    test('Linting some files (Verbose)', async () => {
        ...
        const reporter = new CodeTypoReporterForGithubAction(
            'none',
    ...
    `('Linting checkDotFiles $glob $checkDotFiles', async ({ glob, checkDotFiles, expected }) => {
        ...
        const reporter = new CodeTypoReporterForGithubAction('none', { ...rOptions, verbose: true }, logger);
    ...
    const sampleConfig = resolveFile('fixtures/codetypo.json', sourceDir);
    const sampleConfigTs = resolveFile('fixtures/sampleCode/ts/codetypo.config.yaml', sourceDir);
    ...
    test.each`
        ...
        ${[]}                   | ${['fixtures/sampleCode/ts/codetypo.config.yaml']} | ${{ config: sampleConfig }}   | ${{ files: 1 }}
        ${[]}                   | ${['fixtures/sampleCode/ts/codetypo.config.yaml']} | ${{ config: sampleConfigTs }} | ${{ files: 0 }}
        ${['**/*.ts']}          | ${['fixtures/sampleCode/ts/codetypo.config.yaml']} | ${{ config: sampleConfig }}   | ${{ files: 0 }}
